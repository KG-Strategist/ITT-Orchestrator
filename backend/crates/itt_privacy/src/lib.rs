...
impl SelfHygieneWorker {
    pub fn new(strict_ttl: Duration, corpus_manager: Arc<CorpusManager>) -> Self {
        Self { strict_ttl, corpus_manager }
    }

    pub async fn start_daemon(self) {
        let mut interval = time::interval(self.strict_ttl);
        info!("Self-Hygiene Worker started. Strict TTL: {:?}", self.strict_ttl);

        tokio::spawn(async move {
            loop {
                interval.tick().await;
                info!("Initiating DPDP Compliance Hygiene Sweep...");
                match self.corpus_manager.execute_hard_deletes(self.strict_ttl).await {
                    Ok(deleted) => info!("Hygiene Sweep Complete. {} records permanently purged.", deleted),
